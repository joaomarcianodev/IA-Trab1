...
function generateMap() {
    ...
    WALKABLE_PATH_COORDS.forEach(pos => {
        if (gameMap[pos.y]?.[pos.x]) {
            gameMap[pos.y][pos.x] = Math.random() < 0.7 ? 'plain' : 'rocky';
        }
    });
...
function calculatePath() {
    const waypoints = [START_POS, ...zodiacHouses, END_POS];
    path = [];

    for (let i = 0; i < waypoints.length - 1; i++) {
        const segmentPath = aStarSearch(waypoints[i], waypoints[i + 1]);
        if (segmentPath.length === 0) {
            alert(`Não foi possível encontrar um caminho!`);
            return;
        }
        path.push(...(i === 0 ? segmentPath : segmentPath.slice(1)));
    }
...
function aStarSearch(start, goal) {
    ...
    while (openSet.length > 0) {
        ...
        if (current.x === goal.x && current.y === goal.y) {
            ...
            let node = current;
            while (node) {
                resultPath.unshift({ x: node.x, y: node.y });
                node = node.parent;
            }
            return resultPath;
        }
        ...
        [[1, 0], [-1, 0], [0, 1], [0, -1]].forEach(([dx, dy]) => {
            ...
            if (!terrain || closedSet.has(`${n.x},${n.y}`) || terrain === 'mountain') {
                return;
            }

            const g = current.g + getTerrainCost(terrain);
            const existing = openSet.find(node => node.x === n.x && node.y === n.y);

            if (!existing || g < existing.g) {
                const h = heuristic(n, goal);
                if (!existing) {
                    openSet.push({ ...n, g, h, f: g + h, parent: current });
                } else {
                    Object.assign(existing, { g, h, f: g + h, parent: current });
                }
            }
        });
    }
